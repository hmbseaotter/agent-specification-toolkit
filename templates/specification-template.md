...
- Last swept: [YYYY-MM-DD] @ [spec version] @ D[highest decision]   <!-- staleness marker. Next sweep due at ~8-10 accrued decisions, before publishing, or at phase completion — whichever comes first. Change-based, NOT calendar-based: drift accumulates per decision, not per day, so a monthly reminder fires in quiet weeks and stays silent through the heavy design runs that actually cause drift. Run STEP 8 and update this line, or it silently stops meaning anything. -->
